<template>
  <div class="user-info">
    <!-- 个人信息 -->
    <el-form ref="userInfo" label-width="220px" :model="userInfo" :rules="rules">
      <!-- 工号 入职时间 -->
...
export default {
  data() {
    return {
      ...
      EmployeeEnum, // 员工枚举数据
      userInfo: { username: '' }, // 保存员工简单信息   表单1
      rules: {
        // 为了校验姓名为1-4位
        username: [{ min: 1, max: 4, message: '姓名为1-4位', trigger: 'blur' }]
      },
      formData: { // 保存员工详细基本信息  表单2
        userId: '',
        username: '', // 用户名
        sex: '', // 性别
        mobile: '', // 手机
